...
export default function TeamPill({ teamNumber, teamName }: { teamNumber: number, teamName: string }) {
    ...
    const paddingOfTeamNumber = (teamNumber.toString().length == 1 || (teamName.length >= 15 && teamNameSplitByWords.length <= 2)) ? "mr-2" : "ml-2"

    return (
        <div className="peer flex flex-row items-center justify-center w-40 h-14 bg-blue-600 rounded-2xl transition hover:translate-y-1 hover:bg-red-400">

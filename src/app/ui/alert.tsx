...
export default function Alert({ color, message } : { color: "Error" | "Success", message: string }) {
    ...
    const colorClasses = {
        Error: "text-red-400 border-1.5 border-red-500 bg-slate-800/75",
        Success: "text-green-400 border-1.5 border-green-700 bg-slate-800/75"
    }
    ...
    return (
        <div
            className={`fixed top-12 left-1/2 transform -translate-x-1/2 z-50 flex items-center py-4 px-8 md:mb-4 text-sm border rounded-lg transition-opacity ease-in-out duration-500 ${
                fadingOut ? 'opacity-0' : 'opacity-100'
            } ${colorClasses[color]}`}
            role="alert"
            style={{ width: 'fit-content', maxWidth: '90%' }}  // Ensure that the alert has a reasonable max width
            >
            {iconToUse[color]}

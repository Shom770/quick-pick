...
import { MEDIUM_SCREEN_SIZE, LARGE_SCREEN_SIZE } from "@/app/lib/constants";
            ...
            <div className="flex flex-col w-5/6 md:w-3/4 h-screen md:h-[87.5vh] gap-4">
                <div className="inline-block flex-col items-start justify-center border-b border-[#929292]/50 w-auto basis-1/5">
                    ...
                    <div className="w-full md:w-[25rem] h-20 md:h-16">
                        ...
                        <form action={formAction}>
                            <div className="relative flex flex-row gap-2 w-full h-12 md:h-10">
                                <div className="relative flex flex-1 shrink-0">
                                    <input 
                                        name="eventCode"
                                        className="block rounded-md bg-transparent border border-blue-500/75 placeholder-gray-500 text-base md:text-sm text-white w-full pl-10 focus:outline-none" 
                                ...
                                <button
                                    key="createEvent"
                                    type="submit"
                                    className="flex items-center justify-center w-12 md:w-10 h-12 md:h-10 rounded-md bg-blue-600 hover:bg-blue-500 text-[#0d111b]"
                                >
                                    <PaperAirplaneIcon className="w-6 md:w-5 h-6 md:h-5" />
                                </button>
                            </div>
                        </form>
                    </div>
                    <div className="py-2 md:m-none">
                        { state.errors && 
                            <p className="text-red-400 text-xs md:text-sm">{state.errors[0]}</p>
                        }
                    </div>
                </div>
                <div className="flex flex-col items-start justify-start w-full basis-4/5 gap-2">
                    <div className="inline-block w-full md:w-auto">
                        ...
                        <div className="flex flex-col gap-2 w-full h-36 md:h-24 mt-2">
                            <div className="w-full h-14 md:h-10">
                                <div className="relative flex flex-1 shrink-0 h-12 md:h-10">
                                    <input
                                        className="block rounded-md bg-transparent border border-blue-500/75 placeholder-gray-500 text-base md:text-sm text-white w-full pl-8 focus:outline-none" 
                            ...
                            <Link
                                ...
                                className="flex items-center justify-center gap-2 w-full h-12 md:h-10 rounded-lg text-lg md:text-base -mt-1 md:mt-0 bg-blue-600 hover:bg-blue-500 text-[#0d111b]"
                            >
                    ...
                    <div className="flex items-center justify-center -mt-3 md:my-4 w-full h-[40vh] md:h-[50vh] rounded-xl bg-slate-800">
                                        ...
                                        <div key={`${index}teams`} className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-6 w-full">
                                            {
                                                subList.map(([teamNumber, isInList]) => {
                                                        if (isInList) {
                                                            return (
                                                                <button key={teamNumber} onClick={(_) => setTeams(teamsInEvent.filter((value) => value != parseInt(teamNumber)))} className="mx-auto w-[85%] md:w-40 h-14">
